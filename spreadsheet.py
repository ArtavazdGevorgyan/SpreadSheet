...
class Spreadsheet:
    ...
    def addRow(self, row: int) -> None:
        self.spredsheet = np.insert(self.spredsheet, (row,), Cell, axis=0)

    def removeRow(self, row: int) -> None:
        self.spredsheet = np.delete(self.spredsheet, (row,), axis=0)

    def addColumn(self, col: int) -> None:
        self.spredsheet = np.insert(self.spredsheet, (col,), Cell, axis=1)

    def removeColumn(self, row: int) -> None:
        self.spredsheet = np.delete(self.spredsheet, (1,), axis=1)

    def swapRows(self, row1: int, row2: int) -> None:
        self.spredsheet[[row1, row2]] = self.spredsheet[[row1, row2]]

    def swapColumns(self, col1: int, col2: int) -> None:
        self.spredsheet[:, [col1, col2]] = self.spredsheet[:, [col1, col2]]
